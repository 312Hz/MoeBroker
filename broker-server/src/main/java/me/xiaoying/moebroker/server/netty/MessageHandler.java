...
public class MessageHandler extends SimpleChannelInboundHandler<Serializable> {
    private final BrokerServer server;

    public MessageHandler(final BrokerServer server) {
        this.server = server;
    }
    ...
    private void handleRequestMessage(ChannelHandlerContext ctx, RequestMessage message) {
        ...
        AbstractProcessor processor = this.server.getProcessorManager().getProcessor(object.getClass());
